...
(function() {
    ...
    function hasOption(opt, queryString) {
        ...
        //var m = re.exec(s);
        var m = re.exec(location.search) || re.exec(queryString);
    ...
    var scriptEls = document.getElementsByTagName('script'),
        ...
        themeName = (getQueryParam('theme') || 'triton').replace(/^([-A-Za-z]*)(.*)/, '$1'), //triton  neptune
    ...
    extPackagesRoot = devMode ? (extDir + '/ext') : extDir;
    ...
    if (includeCSS) {
        ...
        var index = themeName.indexOf("-");
        var themeNamecss = themeName;
        if(index>0){
            themeNamecss = themeName.substr(0, index);
        }
        loadCss(extPackagesRoot + '/packages/charts/classic/' + themeNamecss + '/resources/charts-all' + cssSuffix);
        loadCss(extPackagesRoot + '/packages/ux/classic/' + themeNamecss + '/resources/ux-all' + cssSuffix);
        if (Ext.devMode === 2) {
    ...
    svPrefix = useDebug ? '/sv_all-debug' : '/sv_all';

    loadScript(extPackagesRoot + extPrefix + rtlSuffix + '.js');
    loadScript(extPackagesRoot + svPrefix + '.js');
    loadScript(extPackagesRoot + '/classic/locale/locale-zh_CN.js');

    loadCss(extPackagesRoot + '/sv_all.css');
    
    Ext.appDir = extDir.substring(0, extDir.lastIndexOf('/'));
    Ext.actualUrl = function (url) {
        if(url.indexOf('/') == 0) {
            return Ext.appDir + url;
        }
        return url;
    }

    //document.write('<script type="text/javascript" src="' + extPrefix + rtlSuffix + '.js"></script>');
    //document.write('<script type="text/javascript" src="' + extPackagesRoot + '/classic/locale/locale-zh_CN.js"></script>');
